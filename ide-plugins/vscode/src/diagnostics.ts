...
export class SecurityDiagnosticsProvider {
     ...
     */
    private analyzeTerraform(document: vscode.TextDocument, lines: string[]): vscode.Diagnostic[] {
        const diagnostics: vscode.Diagnostic[] = [];
        ...
        lines.forEach((line, index) => {
            ...
            if (/password\s*=\s*["'][^"']+["']/.test(trimmedLine) ||
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Potential hardcoded secret detected. Use environment variables or secret management systems.',
            ...
            if (/publicly_accessible\s*=\s*true/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Resource is publicly accessible. Verify this is intentional and required.',
            ...
            if (/encrypted\s*=\s*false/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Encryption is disabled. Enable encryption for sensitive data at rest.',
                    vscode.DiagnosticSeverity.Error
                );
                diagnostic.source = 'Armadillo Security';
            ...
            if (/cidr_blocks\s*=\s*\["0\.0\.0\.0\/0"\]/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Overly permissive CIDR block (0.0.0.0/0). Restrict access to specific IP ranges.',
                    vscode.DiagnosticSeverity.Warning
                );
                diagnostic.source = 'Armadillo Security';
            ...
            if (/protocol\s*=\s*"HTTP"/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Using HTTP instead of HTTPS. Use HTTPS for secure communication.',
                    vscode.DiagnosticSeverity.Warning
                );
                diagnostic.source = 'Armadillo Security';
     ...
     */
    private analyzeYaml(document: vscode.TextDocument, lines: string[]): vscode.Diagnostic[] {
        const diagnostics: vscode.Diagnostic[] = [];
        ...
        lines.forEach((line, index) => {
            ...
            if (/(password|secret|api_key|token):\s*["']?[a-zA-Z0-9+/=]{8,}["']?/.test(trimmedLine) &&
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Potential hardcoded secret detected. Use Kubernetes secrets or external secret management.',
                    vscode.DiagnosticSeverity.Error
                );
                diagnostic.source = 'Armadillo Security';
                diagnostic.code = 'HARDCODED_SECRET';
            ...
            if (/privileged:\s*true/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Privileged container detected. Avoid running containers in privileged mode unless absolutely necessary.',
                    vscode.DiagnosticSeverity.Error
                );
                diagnostic.source = 'Armadillo Security';
            ...
            if (/hostNetwork:\s*true/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Host network mode enabled. This exposes the host network stack to the container.',
                    vscode.DiagnosticSeverity.Warning
                );
                diagnostic.source = 'Armadillo Security';
            ...
            if (/hostPath:/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'hostPath volume detected. This can expose sensitive host directories to containers.',
                    vscode.DiagnosticSeverity.Warning
                );
                diagnostic.source = 'Armadillo Security';
            ...
            if (/runAsUser:\s*0/.test(trimmedLine)) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    'Container running as root user (UID 0). Use a non-root user for better security.',
                    vscode.DiagnosticSeverity.Warning
                );
                diagnostic.source = 'Armadillo Security';
            ...
            if (trimmedLine.includes('containers:')) {
                ...
                if (!nextFewLines.includes('securityContext:')) {
                    ...
                    const diagnostic = new vscode.Diagnostic(
                        range,
                        'Container missing securityContext. Define security settings for the container.',
            ...
            if (trimmedLine.match(/^\s*-\s*name:/)) {
                ...
                if (!nextFewLines.includes('resources:') || !nextFewLines.includes('limits:')) {
                    ...
                    const diagnostic = new vscode.Diagnostic(
                        range,
                        'Container missing resource limits. Define CPU and memory limits to prevent resource exhaustion.',
                        vscode.DiagnosticSeverity.Information
                    );
                    diagnostic.source = 'Armadillo Security';
